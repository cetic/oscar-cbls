...
object TestGeometryPackingRot extends App with LinearSelectors {
  ...
  val rotationArray:Array[CBLSIntVar] = Array.fill(nbShapes)(null)
  ...
  val placedShapes:Array[AtomicValue[GeometryValue]] = Array.tabulate(nbShapes){ i =>
    ...
    val translation = new Translation(store:Store,coordArray(i)._1,coordArray(i)._2)
    ...
    val rotationFollowedBuTranslation = if(i%2 == 0){
      ...
      new Compose(store:Store, rotation, translation)
    }else{
      translation
    }

    new Apply(store,rotationFollowedBuTranslation, constantShapesAt00(i))
  }
  ...
  */
  ...
  SingleFrameWindow.show(drawing,"searching for a packing without overlap, with rotation",1125,1400)
  ...
  def gradientAndRotate(shapeID:Int) = {
    if(shapeID%2 == 0) {
      gradientOnOneShape(shapeID) step rotate(shapeID)
    }else{
      gradientOnOneShape(shapeID)
    }
  }
  ...
  val search = (Profile(BestSlopeFirst(
    List(
      ...
      Atomic(gradientAndRotate(4), _ > 10) name "grad&Rot4",
      Atomic(gradientAndRotate(5), _ > 10) name "grad5",
      Atomic(gradientAndRotate(6), _ > 10) name "grad&Rot6",
      Atomic(gradientAndRotate(7), _ > 10) name "grad7",
      Atomic(gradientAndRotate(8), _ > 10) name "grad&Rot8",
      Atomic(gradientAndRotate(9), _ > 10) name "grad9",
