...
object UWLPDistributedBench extends ModelingAPI {
  ...
  def createCBLSModel(nbWarehouses: Int,
    ...
    val swapNbs: Seq[Neighborhood] = if (nbWorkers < 4) {
      ...
      for {i <- 0 until nbWorkers} yield swapsNeighborhood(warehouseOpenArray, searchZone1 = {
        val range = divRange.map(_ * nbWorkers + i); () => range
      }, name = s"SwapWarehouse_$i")
    }
