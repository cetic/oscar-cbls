...
abstract class DistributedCombinator(neighborhoods: Array[Neighborhood],
  ...
  override def labelAndExtractRemoteTasks(supervisor: Supervisor,
    ...
    // It's here where the distributed combinator is bound to a supervisor
    this.supervisor = supervisor
    val (_, newAcc, neighborhoods2) = labelAndExtractRemoteNeighborhoodsOutOf(currentID, acc, neighborhoods)
  ...
  (Int, List[RemoteTask], Array[RemoteTask]) = {
    ...
    val toReturnArray = neighborhoods.map(n => {
      val r:RemoteTask = RemoteNeighborhood(currentIDNow, n)
      currentIDNow += 1
      accNow = r :: accNow
      r
    })
