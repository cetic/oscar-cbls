 ...
 *
  ...
  * the VLSN is usable for other problems such as bin packing etc.
  * It's the used neighborhood that matters.
 *
 * We use routing vocabulary in this neighborhood because it is illustrative,
 * and introducing yet another vocabulary would not help so much in the understanding)
  *
  * VLSN will run until no more moves can be found, and at this point,
