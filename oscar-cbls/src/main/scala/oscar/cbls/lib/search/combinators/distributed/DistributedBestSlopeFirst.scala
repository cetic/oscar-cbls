 ...
 * @param minNbRestarts the minimal number of restarts to perform
 ...
 *                  @param timeoutDuration the duration before a timeout, default is infinite
 */
class DistributedRestartFromBest(baseSearch:Neighborhood,
  ...
  override def getMove(obj: Objective, initialObj: Long, acceptanceCriteria: (Long, Long) => Boolean): SearchResult = {
    ...
    case class WrappedSearchTimeout() extends WrappedData
    ...
    def startSearch(visu:Option[ActorRef[SearchProgress]],context:ActorContext[WrappedData]): Behavior[WrappedData] = {
      ...
      timeoutDuration match {
        case _: Infinite => ;
        case f: FiniteDuration =>
          context.log.info(s"Timeout de: $timeoutDuration")
          context.scheduleOnce(f, context.self, WrappedSearchTimeout())
      }
      next(
    ...
    def next(runningSearchIDsAndIsItFromBestSoFar:SortedMap[Long,Boolean],
      ...
      Behaviors.receive { (context, command) =>
        command match {
          ...
          case WrappedSearchTimeout() =>
            context.log.info(s"Timeout, canceling ${runningSearchIDsAndIsItFromBestSoFar.size -1} ongoing searches finalOBj:$bestObjSoFar")
            for(searchID <- runningSearchIDsAndIsItFromBestSoFar.keys){
              supervisor.supervisorActor ! CancelSearchToSupervisor(searchID)

            }

            nextCompleting(runningSearchIDsAndIsItFromBestSoFar.size ,
              bestObjSoFar,
              bestMoveSoFar,
              display,
              nbCompletedRestarts +1,
              context,
              forceStop = true)
    ...
    def nextCompleting(nbRunningSearches:Int,
                       ...
                       nbCompletedRestarts:Int,
                       context:ActorContext[WrappedData],
                       forceStop:Boolean = false): Behavior[WrappedData] = {
      if(forceStop){
        context.log.info(s"forceStop; obj: $bestObjSoFar")
