...
class VehicleSegmentsTests extends AnyFunSuite with Matchers {
  ...
  test("Vehicle of segment is correctly initialized") {
    ...
    model.close()
    vrp.routes := IntSequence(List.from(0 to 5))
    model.propagate()

    val segmentsOfVehicle = VehicleSegments(vrp, 0)
  ...
  test("Removes one point") {
    ...
    model.propagate()
    val segmentsOfVehicle = VehicleSegments(vrp, 0)
    val toRemoveExp       = vrp.routes.value().explorerAtAnyOccurrence(3).get
  ...
  test("Inserts a new node") {
    ...
    model.propagate()
    val segmentsOfVehicle = VehicleSegments(vrp, 0)
    val insertAfterExp    = vrp.routes.value().explorerAtAnyOccurrence(3).get
  ...
  test("Inserts two adjacent new nodes") {
    ...
    vrp.routes := IntSequence(List.from(0 to 5))
    model.propagate()
    var segmentsOfVehicle = VehicleSegments(vrp, 0)
    var insertAfterExp    = vrp.routes.value().explorerAtAnyOccurrence(3).get
    vrp.routes.insertAfterPosition(7, insertAfterExp)
    segmentsOfVehicle = segmentsOfVehicle.insertNode(7, insertAfterExp, 0)

    insertAfterExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(7).get
    vrp.routes.insertAfterPosition(9, insertAfterExp)
  ...
  test("Moves segments on the same vehicle") {
    val model = new Store()
    val vrp   = VRP(model, 20, 1)
    model.close()
    vrp.routes := IntSequence(List.from(0 to 15))
    model.propagate()
    var segmentsOfVehicle = VehicleSegments(vrp, 0)

    var toRemoveExp = vrp.routes.value().explorerAtAnyOccurrence(3).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle = segmentsOfVehicle.removeNode(toRemoveExp, 0)._1

    toRemoveExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(10).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle = segmentsOfVehicle.removeNode(toRemoveExp, 0)._1

    var insertAfterExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(5).get
    vrp.routes.insertAfterPosition(17, insertAfterExp)
    segmentsOfVehicle = segmentsOfVehicle.insertNode(17, insertAfterExp, 0)

    insertAfterExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(7).get
    vrp.routes.insertAfterPosition(16, insertAfterExp)
    segmentsOfVehicle = segmentsOfVehicle.insertNode(16, insertAfterExp, 0)

    val fromExp = vrp.routes.pendingValue.explorerAtPosition(2).get
    val toExp   = vrp.routes.pendingValue.explorerAtPosition(8).get
    insertAfterExp = vrp.routes.pendingValue.explorerAtPosition(12).get
    vrp.routes.move(fromExp, toExp, insertAfterExp, flip = false)
  ...
  test("Moves and flips segments on the same vehicle") {
    ...
    val toExp   = vrp.routes.pendingValue.explorerAtPosition(8).get
    insertAfterExp = vrp.routes.pendingValue.explorerAtPosition(12).get
    vrp.routes.move(fromExp, toExp, insertAfterExp, flip = true)
    val (sourceSeg, targetSeg) = VehicleSegments.moveSegments(
      0,
      0,
      segmentsOfVehicle,
  ...
  test("Move segments to another vehicle") {
    val model = new Store()
    val vrp   = VRP(model, 50, 2)
    model.close()
    vrp.routes := IntSequence(List.from(0 to 20 by 2) ::: List.from(1 to 21 by 2))
    model.propagate()
    var segmentsOfVehicle0 = VehicleSegments(vrp, 0)
    var segmentsOfVehicle1 = VehicleSegments(vrp, 1)

    var toRemoveExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(6).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle0 = segmentsOfVehicle0.removeNode(toRemoveExp, 0)._1

    toRemoveExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(16).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle0 = segmentsOfVehicle0.removeNode(toRemoveExp, 0)._1

    var insertAfterExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(10).get
    vrp.routes.insertAfterPosition(42, insertAfterExp)
    segmentsOfVehicle0 = segmentsOfVehicle0.insertNode(42, insertAfterExp, 0)

    val posOf1 = vrp.routes.pendingValue.positionOfAnyOccurrence(1).get

    toRemoveExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(7).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle1 = segmentsOfVehicle1.removeNode(toRemoveExp, posOf1)._1

    toRemoveExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(15).get
    vrp.routes.remove(toRemoveExp)
    segmentsOfVehicle1 = segmentsOfVehicle1.removeNode(toRemoveExp, posOf1)._1

    val fromExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(4).get
    val toExp   = vrp.routes.pendingValue.explorerAtAnyOccurrence(18).get
    insertAfterExp = vrp.routes.pendingValue.explorerAtAnyOccurrence(11).get
    vrp.routes.move(fromExp, toExp, insertAfterExp, flip = false)
    ...
    var startExp = vrp.routes.value().explorerAtAnyOccurrence(0).get
    for (seg <- newSeg0.segments ::: newSeg1.segments) {
      val start = startExp.value

      start must equal(seg.startNode())

      val endExp = vrp.routes.value().explorerAtPosition(startExp.position + seg.length() - 1).get
