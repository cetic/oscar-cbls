...
class PropagationStruct2UnitTestSuite extends AnyFunSuite with Matchers {
  ...
  val inv3 = new TestInvariantElement(struct)
  val out3_1 = new TestVariableElement(struct)
  inv3.registerStaticAndDynamicDependency(out2_2)
  inv3.registerStaticAndDynamicDependency(out1_2)
  out3_1.setDefiningInvariant(inv3)
  ...
  struct.registerForPartialPropagation(out3_1)
  ...
  test("Unit test: after a partial propagation, if a new partial propagation is triggered, the postponed variables that are concerned by this propagation are updated") {
    ...
    struct.partialPropagation(out3_1,false)
