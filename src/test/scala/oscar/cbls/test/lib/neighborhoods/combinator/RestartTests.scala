...
import oscar.cbls.lib.neighborhoods.combinator.Restart
import oscar.cbls.lib.neighborhoods.routing.{RandomizeRoutes, TwoOpt}
...
class RestartTests extends AnyFunSuite with Matchers {
  ...
  ignore("Search with random restart") {
    ...
    val twoOpt =
      TwoOpt(vrs, relevantStartSegment, selectFlippedSegmentBehavior = LoopBehavior.first())
    val restartNeigh = RandomizeRoutes.shuffle(vrs, 3, rng)
