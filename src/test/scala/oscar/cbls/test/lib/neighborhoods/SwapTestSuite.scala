...
import oscar.cbls.lib.neighborhoods.Swap

import scala.util.Random

class SwapTestSuite extends AnyFunSuite {
  ...
  ignore("Swap works as expected with First loop") {
    val (input, objective) = getTestBasicModel()

    val search = Swap(input)
  ...
  ignore("Swap works as expected with Best loop") {
    val (input, objective) = getTestBasicModel()
