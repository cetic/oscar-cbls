...
import oscar.cbls.lib.neighborhoods.routing.RemovePoint
...
class RemovePointTestSuite extends AnyFunSuite with Matchers {
  ...
  test("RemovePoint works as expected on a simple example") {
    ...
    val search = RemovePoint(vrs, relevantRemove)
  ...
  ignore("RemovePoint with hot restart") {
    ...
    val search =
      RemovePoint(vrs, relevantRemove, selectNodesToRemoveBehavior = LoopBehavior.first())
    search.verbosityLevel = 4
