...
class AsymmetricalMatrixUnitTests extends AnyFunSuite with Matchers {

  test("TotalRouteLength is correctly initialized") {
    val model = new Store()
    val vrs   = VRS(model, n = 6, v = 2)
    ...
    val routeLength = TotalRouteLength(vrs, matrix)
    model.close()
    val routes =
      List(
        List(0, 1, 5, 2, 4),
    ...
    for (i <- expectedLength.indices) {
      vrs.routes := IntSequence(routes(i))
      model.propagate()
  ...
  test("Total Route Length works when inserting points in an empty route") {
    ...
    TotalRouteLength(vrs, distanceMatrix)
    model.close()

    val routes = vrs.routes
  ...
  test("Total Route Length works when moving and flipping segment") {
    val model = new Store(debugLevel = 3)
    val vrs   = VRS(model, 10, 2)
    val _     = TotalRouteLength(vrs, distanceMatrix)
    model.close()
