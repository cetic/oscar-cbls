...
class PiecewiseLinearFun(
  ...
  def swapAdjacentZonesShiftFirst(
    ...
    val f2 = LinearTransform(-widthZone2, opposite = false)
    val f3 = LinearTransform(widthZone2, opposite = false)
    ...
    val transformWithZone1Shifted =
      transformReadyForShiftOfZone1.updateDelta(
        ...
        endZone1Included,
        widthZone2,
        (p: Pivot) => new Pivot(p.fromValue + widthZone2, if (p.f.opposite) f3(p.f) else f2(p.f))
      )
  ...
  /** flips the function between the given bounds. does not add closing pivots before
    ...
    * @return
    */
  def flipFunctionInInterval(startZoneIncluded: Int, endZoneIncluded: Int): PiecewiseLinearFun = {
    // a pivot must be added after the end of update1 if 2 is the first zone
    ...
    val flippedPivots =
      flipSubfunction(collectedPivotsForwardOrder, endZoneIncluded, endZoneIncluded)
  ...
  @tailrec
  private def flipSubfunction(
    pivotList: List[Pivot],
    endOfLastPivotMoving: Int,
  ...
  def swapAdjacentZonesShiftSecond(
    ...
    val f2 = LinearTransform(widthZone1, opposite = false)
    val f3 = LinearTransform(-widthZone1, opposite = false)
    ...
    val transformWithZone2Shifted =
      transformReadyForShiftOfZone2.updateDelta(
        ...
        endZone2Included,
        -widthZone1,
        (p: Pivot) => new Pivot(p.fromValue - widthZone1, if (p.f.opposite) f3(p.f) else f2(p.f))
      )
  ...
  def updateForCompositionBefore(
    fromIncluded: Int,
    toIncluded: Int,
    additionalFAppliedBefore: LinearTransform
  ): PiecewiseLinearFun = {
        ...
        updatedTransform.biggestLowerOrEqual(toIncluded) match {
          case None =>
            if (pivotStartingJustAfterToIncluded.f.isIdentity)
              updatedTransform.remove(toIncluded + 1)
  ...
  private def myUpdateForCompositionBefore(
    ...
    val isAdditionaFNegativeSlope = additionalFAppliedBefore.opposite
    ...
    var positionOfPivotApplyingOnCurrentIncludedFromAfterAdditionalF =
      transformation.biggestLowerOrEqual(currentIncludedFromAfterAdditionalF) match {
        case None =>
          None // no pivot applying at this point, consider identity function... until when?
      ...
      positionOfPivotApplyingOnCurrentIncludedFromAfterAdditionalF match {
        case None => // no pivot at current point, so identity is assumed
          val (nextCurrentIncludedFrom, nextPivotPosition) = if (isAdditionaFNegativeSlope) {
            ...
            val nextPivotPosition: Option[RedBlackTreeMapExplorer[Pivot]] =
              transformation.smallestPosition
            (
              nextPivotPosition match {
                case None           => toIncluded + 1
                case Some(position) => additionalFAppliedBefore.unApply(position.value.fromValue)
              },
              nextPivotPosition
            )
          }
          ...
          if (
            ...
            currentTransformation = currentTransformation.insert(
              currentFromIncluded,
              new Pivot(currentFromIncluded, newF)
            )
          }
  ...
  /** @param addedPivot
    * @param transform
    * @return
    *   transform. there will always be a pivot at the addedPivot position.
    */
  private def addRedundantPivotAt(
    ...
    transform.biggestLowerOrEqual(addedPivot) match {
      case Some((key, pivot)) =>
        if (key == addedPivot)
          transform // we do not add a redundant pivot because there is already a pivot here
        else transform.insert(addedPivot, new Pivot(addedPivot, pivot.f))
      case _ =>
        transform.insert(
          addedPivot,
          new Pivot(addedPivot, LinearTransform.identity)
        ) // implicitly, it was the identity function here, so add the pivot
    }
  }

  /** removes all pivots between fromIncluded and toIncluded also adds a pivot at toIncluded+1 if
    * necessary to ensure that values starting at toIncluded+1 onwards are not impacted
    */
...
class Pivot(val fromValue: Int, val f: LinearTransform) {
  ...
  def flip(endX: Int): Pivot = {
    if (f.opposite) {
      new Pivot(fromValue, LinearTransform(f(endX) - fromValue, opposite = false))
    } else {
      new Pivot(fromValue, LinearTransform(fromValue + f(endX), opposite = true))
    }
  }
}
