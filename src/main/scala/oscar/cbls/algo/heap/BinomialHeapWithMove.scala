...
/** This is a binary heap that is less efficient than the [[oscar.cbls.algo.heap.BinomialHeap]]. It
  * offers more operations, such as delete and update value. smallest first
  * @param getKey
  *   a function that returns an integer for each element inserted i nthe heap this value is used to
  *   sort the heap content
  * @param maxsize
  *   the maximum number of elements that can be inserted in this heap
  * @param X
  ...
  *   renaud.delandtsheer@cetic.be
  */
class BinomialHeapWithMove[T](getKey: T => Long, val maxsize: Int)(
  ...
  private def pushDown(startposition: Int): Int = {
    ...
    while (true)
      if (
        leftChild(position) < size && getKey(heapArray(position)) > getKey(
          heapArray(leftChild(position))
        )
      ) {
        // examiner aussi left child
        if (
          rightChild(position) < size && getKey(heapArray(rightChild(position))) < getKey(
            heapArray(leftChild(position))
          )
        ) {
          // c'est avec le right child qu'il faut inverser
        ...
        rightChild(position) < size && getKey(heapArray(position)) > getKey(
          heapArray(rightChild(position))
        )
      ) {
        // only consider right child
  ...
  /** removes one elem from the heap if present
    * @param elem
    * @return
    *   true if it was in the heap, false otherwise
    */
...
/** beware that this heap does not delete references that are passed to it, so GC will not be able
  ...
  * @tparam T
  */
class BinomialHeapWithMoveExtMem[T](
  getKey: T => Long,
  val maxsize: Int,
  position: scala.collection.mutable.Map[T, Int]
)(implicit val A: Ordering[T], implicit val X: Manifest[T]) {
  ...
  private def pushDown(startposition: Int): Int = {
    ...
    while (true)
      if (leftChild(position) < size && positionKey > getKey(heapArray(leftChild(position)))) {
        ...
        rightChild(position) < size && positionKey > getKey(heapArray(rightChild(position)))
      ) {
        // only consider right child
        swapPositions(position, rightChild(position))
        position = rightChild(position)
  ...
  /** removes one elem from the heap if present
    * @param elem
    * @return
    *   trus if it was in the heap, false otherwise
    */
  def deleteIfPresent(elem: T): Boolean = {
    if (contains(elem)) {
      delete(elem)
...
class BinomialHeapWithMoveInt(getKey: Int => Int, val maxsize: Int, val maxKey: Int) {
  ...
  /** removes one elem from the heap if present
    * @param elem
    *   the element to seek in the heap
    * @return
    *   true if it was in the heap, false otherwise
    */
...
class BinomialHeapWithMoveLong(getKey: Int => Long, val maxsize: Int, val maxKey: Int) {
  ...
  /** removes the smallest element and returns its value
    * @return
    *   the value of the deleted element (the smallest one)
    */
  def removeFirst(): Int = {
    val toreturn: Int = heapArray(0)
    swapPositions(0, size - 1)
