...
/** SearchProfiler specialized for composition type combinator. It adds the possibility to handle
...
case class CompositionProfiler(
  ...
  // Pauses the time tracker of the left Neighborhood exploration.
  def leftExplorationPaused(): Unit =
    left.get.searchProfiler().get.explorationPaused()

  // Resumes the time tracker of the left Neighborhood exploration.
  def leftExplorationResumed(): Unit =
    left.get.searchProfiler().get.explorationResumed()
  ...
  def mergeDynProfiler(profiler: SearchProfiler = currentRightProfiler): Unit = {
    if (!dynNeighborhoodProfiler.exists(x => x eq profiler)) {
      ...
      else if (profiler != null) dynNeighborhoodProfiler = dynNeighborhoodProfiler :+ profiler
    }
  }
}
