...
/** Profiles and computes the number of occurrences of an event over iterations.
  ...
  *   Whether the instance must start with an occurrence.
  * @param startIncOccurrence
  *   Whether the instance must start at the second iteration.
  */
private[profiling] case class NbOccurrencesPerIteration(
  ...
  private var _occurrences: Int       = if (startIncOccurrence) 1 else 0
  ...
  private var _iterations: Int        = if (startIncIteration) 2 else 1
