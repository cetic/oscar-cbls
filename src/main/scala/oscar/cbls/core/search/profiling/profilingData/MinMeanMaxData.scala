...
/** Profiles and computes the min, mean and max value of a variable during a search.
  ...
  * ex: Exhaust time
  */
private[profiling] case class MinMeanMaxData(initValue: Long) extends CombinatorProfilingData {
  private var _min: Long  = initValue
  private var _max: Long  = initValue
  private var _count: Int = 1
  private var _sum: Long  = initValue

  // Returns minimal recorded value of the profiled data.
  def min: Long  = _min
  // Returns maximal recorded value of the profiled data.
  def max: Long  = _max
