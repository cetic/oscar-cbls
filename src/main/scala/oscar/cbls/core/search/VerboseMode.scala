...
/** This class handles the verbosity aspect of the search, meaning the information displayed during
  ...
  */
class VerboseMode(var verbosityLevel: Int) {
  ...
  final def searchEnded(endValue: Long, moveCount: Int): Unit = {
    ...
    if (verbosityLevel >= 1) {
      val totalDurationNs = System.nanoTime() - searchStartAt
      val duration        = Duration.fromNanos(totalDurationNs)
