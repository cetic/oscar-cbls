...
/** An interface that provides methods to handle search procedure.
  ...
  *
  * For instance :
  *   - Assign => Upon calling [[Neighborhood.doAllMoves]], this search procedure will assign new
  *     values to a variable. Until no value are left to test.
  *
  *   - Assign(A: IntVariable) dynAndThen Assign(B: IntVariable) ==> This one will assign a value to
  *     A and then to B and evaluate the combined movement.
  *
  * @param _name
...
abstract class Neighborhood(_name: String) {
  ...
  /** Does as many moves as possible or until the shouldStop condition is met.
    *
    * @param objective
    *   The Objective of the search (minimizing, maximizing...)
    * @param shouldStop
  ...
  /** Does as many moves as possible or until the shouldStop condition is met.
    ...
    *   moves.
    * @return
    *   All the performed moves
    */
  def getAllMoves(objective: Objective, shouldStop: Int => Boolean = _ => false): List[Move] = {
    require(
      objective.objValue.model.isClosed,
      "Cannot start the search, the model of your " +
        "objective variable is not closed."
    ...
    var moveCount: Int             = 0
    var noMoreMove: Boolean        = false
    var improvingMoves: List[Move] = List()
    objective.verboseMode_=(this._verboseMode)
    objective.startSearch()
    ...
    while (!shouldStop(moveCount) && !noMoreMove) {
      ...
      getMoveResult match {
        ...
        case mf: MoveFound =>
          moveCount += 1
          improvingMoves = mf.move :: improvingMoves
          objective.commitMove(mf.move)
      }
    }
    improvingMoves.reverse
  }
  ...
  protected def setObjectiveVerboseMode(objective: Objective, verbosityLevel: Int): Unit =
    objective.verboseMode.verbosityLevel = verbosityLevel
