...
class IdentitySeqInvariant(fromValue:SeqVariable, toValue:SeqVariable)
  ...
  def digestChanges(changes:SeqUpdate): Unit ={
    changes match{
      case SeqUpdateInsert(value:Int,insertAfterPositionExplorer:IntSequenceExplorer,prev:SeqUpdate) =>
        digestChanges(prev)
        toValue.insertAfterPosition(value,insertAfterPositionExplorer,Some(changes.newValue))
      case SeqUpdateMove(fromIncludedExplorer:IntSequenceExplorer,toIncludedExplorer:IntSequenceExplorer,afterExplorer:IntSequenceExplorer,flip:Boolean,prev:SeqUpdate) =>
        digestChanges(prev)
        toValue.move(fromIncludedExplorer,toIncludedExplorer,afterExplorer,flip,Some(changes.newValue))
