...
/** Abstract class for [[NaiveRoutingConstraint]] where the computations along the sequence are
  ...
  */
abstract class BackwardNaiveRoutingConstraint[U: Manifest](
  vrp: VRP,
  ...
  override protected def getNextPointOfSegmentToUpdate(
    ...
    seg match {
      case PrecomputedSubSequence(startNode, _, _) if exp.value != startNode =>
        Some(exp.prev)
      case FlippedPreComputedSubSequence(startNode, _, _) if exp.value != startNode =>
        Some(exp.prev)
      case _ => None
    }
  }

  override protected def firstPointToUpdateOnSegment(seg: Segment): Int = seg.endNode()

  override protected def lastPointToUpdateOnSegment(seg: Segment): Int = seg.startNode()

  override protected def orderSegments(vehicleSegments: VehicleSegments): List[Segment] =
    vehicleSegments.segments.reverse
}
