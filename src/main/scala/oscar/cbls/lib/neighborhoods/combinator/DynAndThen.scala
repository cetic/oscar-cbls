...
object DynAndThen {
  ...
  /** This Combinator allows to build [[CompositeMove]]s.
    *
    * A [[CompositeMove]] is a move composed of two sub-[[Move]]s. The key point is that the two
...
/** This Combinator allows to build [[CompositeMove]]s.
  *
  * A [[CompositeMove]] is a move composed of two sub-[[Move]]s. The key point is that the two moves
...
class DynAndThen(left: Neighborhood, right: Move => Neighborhood)
  ...
  override protected[this] def exploreCombinator(objective: Objective): SearchResult = {
    ...
    searchProfiler().foreach(_.mergeDynProfiler())
    searchResult
  }
