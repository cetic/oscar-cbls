...
/** Companion object of the [[InsertPointUnroutedFirst]] class */
object InsertPointUnroutedFirst {
  ...
  def apply(
    ...
    name: String = "InsertPointUnroutedFirst",
  ...
  ): InsertPointUnroutedFirst = new InsertPointUnroutedFirst(
    vrs,
    nodesToInsert,
...
/** Neighborhood which inserts a new value in a sequence. It first selects the new value to insert
  ...
  *   Set the use of a [[oscar.cbls.algo.search.HotRestart]] mechanism.
  */
class InsertPointUnroutedFirst(
  vrs: VRS,
  nodesToInsert: () => Iterable[Int],
      ...
      for (insertPoint <- insertPointIterator) {
        ...
        insertPointExplorer match {
          ...
          case Some(insertAfterExplorer) =>
            vrs.routes.insertAfterPosition(nodeToInsert, insertAfterExplorer) // Commits the move
            searchProfiler().foreach(x => x.neighborSelected())

            exploration.checkNeighborWP(objValue =>
              InsertPointMove(vrs.routes, nodeToInsert, insertAfterExplorer, objValue, name)
