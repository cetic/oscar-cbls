...
/** Companion object of the [[Swap]] class. */
object Swap {
...
/** Neighborhood that find two [[oscar.cbls.core.computation.integer.IntVariable]] from an input
  ...
  *   `None` is provided, all the variables are considered.
  * @param secondSearchZone
  *   A subset of indices of `vars` to consider for the second variable of the Swap operation. These
      ...
      while (secondIterator.hasNext) {
        ...
        if (
          ...
          exploration.checkNeighborWP(objValue =>
            SwapMove(firstVar, indexOfFirstVariable, secondVar, secondIndex, objValue, this.name)
          )
          // Rollbacks the move
          firstVar :=: secondVar
