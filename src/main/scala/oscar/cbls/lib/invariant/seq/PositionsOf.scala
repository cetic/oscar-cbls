...
import oscar.cbls.core.computation.{ChangingIntValue, ChangingSeqValue, ChangingSetValue, Domain, IntDomainHelper, IntNotificationTarget, IntValue, SeqNotificationTarget, SeqUpdate, SeqValue, SetInvariant}
...
class PositionsOf(v: SeqValue, a:IntValue)
  extends SetInvariant(v.value.positionsOfValue(a.valueInt).foldLeft(SortedSet.empty[Int])((acc:SortedSet[Int],i:Int) => acc + i) , Domain(0 , IntDomainHelper.safeAdd(v.max,1)))
...
class PositionsOfConst(v: SeqValue, a:Int)
  extends SetInvariant(v.value.positionsOfValue(a).foldLeft(SortedSet.empty[Int])((acc:SortedSet[Int],i:Int) => acc + i), Domain(0 , IntDomainHelper.safeAdd(v.max,1)))
