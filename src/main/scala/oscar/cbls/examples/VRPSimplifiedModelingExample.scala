...
object VRPSimplifiedModelingExample {
  def main(args: Array[String]): Unit = {
    ...
    val totalRouteLength: IntVariable =
      Routing.totalRouteLength(vrs, distanceMatrix, matrixIsSymmetrical = true)
    ...
    val n1: Neighborhood =
      Routing.insertPointUnroutedFirst(vrs, () => vrs.unroutedNodes, node => kFirst(node))
    val n2: Neighborhood =
      Routing.onePointMove(vrs, () => vrs.routedWithoutVehicles.value(), node => kFirst(node))
    val n3: Neighborhood = Routing.twoOpt(vrs, () => vrs.routedWithoutVehicles.value())
    val n4: Neighborhood = Routing.threeOptMovedSegmentFirst(
      vrs,
    ...
    val search: Neighborhood = m.search.bestSlopeFirst(List(n1, n2, n3, n4))

    // Try population-based search
//    val pbs = m.search.populationBased((it, _) =>
