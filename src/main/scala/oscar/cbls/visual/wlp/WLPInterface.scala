...
class WLPInterface(nbWarehouses: Int,
  ...
  case class SortedSetResult(ss: SortedSet[Int]) extends MoveResult
  ...
  private def sceneWLP(openWS: SortedSet[Int]): Scene = new Scene {
    ...
    val circles: Array[Circle] = Array.tabulate(nbDelivery) { i =>
      ...
      val whp = warehousesPositions(wlpMap.nearestOpenWarehouse(i))
      val distance = sqrt(pow((dlv._1-whp._1).toDouble, 2d) + pow((dlv._2-whp._2).toDouble, 2d))
      val cxy = new Circle {
  ...
  def update(move: MoveOperation, newTime: Double, newRemainingRetrys: Int): Unit = {
    ...
    move match {
      case Move(res) =>
        res match {
          case SortedSetResult(openWs) =>
            wlpMap.openWarehouses = openWs
            println(s"$newTime s: Found move : open ws = ${openWs.size}")
            objDisplay.updateObj(newTime, openWs.size)
            Platform.runLater {
              logDisplay.addLog(s"$newTime s", "Found Move :", s"open ws = ${openWs.size}")
  ...
  override def start(): Unit = {
    objDisplay = new ObjectiveFunctionDisplay(titleWindow = "WLP Drawing - Open Warehouses")
