...
import oscar.cbls.core.search.profiling.{CombinatorProfiler, CompositionProfiler, SearchProfiler}
...
abstract class ProfilingTableNode(
  ...
  // If this node is within the left or right branch of a DynAndThen, it's gain and slope are irrelevant.
  private[profiling] val withinLeftOrRightStub: Boolean =
    parent.isDefined && (parent.get.profiler
      .isInstanceOf[CompositionProfiler] || parent.get.withinLeftOrRightStub)
  val depth: Int = if (parent.nonEmpty) parent.get.depth + 1 else 0
  // Whether this node should be displayed (graphical display context)
  var isDisplayed: Boolean = parent.isEmpty

  private val profilerCommonData: List[String] =
    profiler.collectThisProfileData(hideGainRelatedData = withinLeftOrRightStub)

  // Whether this node is expanded (graphical display context)
